...
describe('Authentication', function () {
	...
	describe('without auth', () => {
		describe('reading + getInfo', () => {
			it('getInfo should fail on a private doc', async () => {
				return expect(docs['private'].getInfo()).to.be.rejectedWith('Sheet is private. Use authentication or make public.');
			});
			
			it('should fail on a private doc', async () => {
				return expect(docs['private'].getRows(1)).to.be.rejectedWith('Sheet is private. Use authentication or make public.');
			});
	...
	describe('authentication', () => {
		...
		it('should fail if the key is no good', async () => {
			return expect(docs['private'].useServiceAccountAuth({
		...
		it('should fail if the email and key do not match', async () => {
			...
			return expect(docs['private'].useServiceAccountAuth(bad_creds))
				.to.be.rejected;
		});
		...
		it('should fail if the path is invalid', async () => {
			...
			return expect(docs['private'].useServiceAccountAuth(creds_file_path))
				.to.be.rejected;
		});
	});
