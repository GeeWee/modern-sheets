...
export const GoogleSpreadsheet: any = function (ss_key, auth_id, options) {
	const self = this;
...
var SpreadsheetWorksheet = function( spreadsheet, data ){
  ...
  this.bulkUpdateCells = function(cells, cb) {
    if ( !cb ) cb = function(){};
	  ...
	  const entries = cells.map(function (cell, i) {
...
var SpreadsheetCell: any = function( spreadsheet, worksheet_id, data ){
  ...
  self.save = function(cb) {
    ...
    self._needsSave = false;
	  ...
	  const edit_id = 'https://spreadsheets.google.com/feeds/cells/key/worksheetId/private/full/R' + self.row + 'C' + self.col;
	  let data_xml =
		  '<entry><id>' + self.id + '</id>' +
